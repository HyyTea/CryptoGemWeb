...
<head>
    ...
    <div class="row" style="margin-top: 4%">
        <div class="card post-header">
            <?php
            $pid = intval($_GET['nid']);
            $query = mysqli_query($con, "SELECT tblpopularposts.PostTitle AS posttitle, tblpopularposts.PostImage, tblcategory.CategoryName AS category, tblcategory.id AS cid, tblsubcategory.Subcategory AS subcategory, tblpopularposts.PostDetails AS postdetails, tblpopularposts.PostingDate AS postingdate, tblpopularposts.PostUrl AS url FROM tblpopularposts LEFT JOIN tblcategory ON tblcategory.id = tblpopularposts.CategoryId LEFT JOIN tblsubcategory ON tblsubcategory.SubCategoryId = tblpopularposts.SubCategoryId WHERE tblpopularposts.id = '$pid'");
            while ($row = mysqli_fetch_array($query)) {
            ?>
            <div class="overflow-hidden" style="height: 100%; max-height: 450px">
              <img class="card-img-top w-100 h-auto" src="admin/popularpostimages/<?php echo htmlentities($row['PostImage']); ?>" alt="<?php echo htmlentities($row['posttitle']); ?>">
    ...
    <?php
      ...
      $query4 = mysqli_query($con, "SELECT tblpopularposts.id AS pid4,
          tblpopularposts.PostTitle AS posttitle4,
          ...
          tblpopularposts.PostDetails AS postdetails4,
          tblpopularposts.PostingDate AS postingdate4,
          tblpopularposts.PostUrl AS url4 
          ...
          WHERE tblpopularposts.Is_Active = 1 AND tblpopularposts.id != '$pid' 
    ...
    <div class="row" style="gap: 20px;">
        ...
        foreach (array_slice($posts2, 0, 5) as $index => $post2) {
            ...
            echo '<a href="news-populardetails.php?nid=' . htmlentities($post2['pid4']) . '" class="text-decoration-none text-dark" style="display: block;">';
            ...
            echo '<img src="admin/popularpostimages/' . htmlentities($post2['PostImage']) . '" class="w-100 h-100" style="object-fit: cover;" alt="' . htmlentities($post2['posttitle4']) . '">';
            echo '</div>';
