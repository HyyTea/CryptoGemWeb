...
<head>
    ...
    <div class="row" style="margin-top: 4%">
        <div class="card post-header">
            <?php
            $pid = intval($_GET['nid']);
            $query = mysqli_query($con, "SELECT tblmainposts.PostTitle AS posttitle, tblmainposts.PostImage, tblcategory.CategoryName AS category, tblcategory.id AS cid, tblsubcategory.Subcategory AS subcategory, tblmainposts.PostDetails AS postdetails, tblmainposts.PostingDate AS postingdate, tblmainposts.PostUrl AS url FROM tblmainposts LEFT JOIN tblcategory ON tblcategory.id = tblmainposts.CategoryId LEFT JOIN tblsubcategory ON tblsubcategory.SubCategoryId = tblmainposts.SubCategoryId WHERE tblmainposts.id = '$pid'");
            while ($row = mysqli_fetch_array($query)) {
            ?>
            <div class="overflow-hidden" style="height: 100%; max-height: 450px">
              <img class="card-img-top w-100 h-auto" src="admin/mainpostimages/<?php echo htmlentities($row['PostImage']); ?>" alt="<?php echo htmlentities($row['posttitle']); ?>">
    ...
    <?php
      ...
      $query4 = mysqli_query($con, "SELECT tblmainposts.id AS pid4,
          tblmainposts.PostTitle AS posttitle4,
          ...
          tblmainposts.PostDetails AS postdetails4,
          tblmainposts.PostingDate AS postingdate4,
          tblmainposts.PostUrl AS url4 
          FROM tblmainposts 
          ...
          WHERE tblmainposts.Is_Active = 1 AND tblmainposts.id != '$pid' 
          ORDER BY tblmainposts.id DESC  
