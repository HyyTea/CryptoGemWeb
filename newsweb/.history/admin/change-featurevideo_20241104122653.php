...
{ 
    ...
    if (isset($_POST['update'])) {
        ...
        $uploadDir = 'featurepostvideo/'; // Thư mục để lưu video đã upload
        ...
        if (isset($_FILES['uploadvideo']) && $_FILES['uploadvideo']['error'] == 0) {
            $fileTmpPath = $_FILES['uploadvideo']['tmp_name'];
            ...
            // Kiểm tra định dạng file video (có thể thêm các định dạng khác nếu cần)
            $allowedExts = ['mp4', 'avi', 'mov', 'wmv'];
            if (in_array($fileExtension, $allowedExts)) {
                ...
                if (move_uploaded_file($fileTmpPath, $dest_path)) {
                    $postVideo = $newFileName; // Lưu tên file video đã upload vào biến $postVideo
                } else {
                    echo "<script>alert('Error moving the uploaded file.');</script>";
                    return;
                }
            } else {
                echo "<script>alert('Invalid file format. Please upload a video file.');</script>";
                return;
            }
        }
