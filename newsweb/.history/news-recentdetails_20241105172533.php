...
<head>
    ...
    <div class="row" style="margin-top: 4%">
        <div class="card post-header">
            <?php
            $pid = intval($_GET['nid']);
            $query = mysqli_query($con, "SELECT tblrecentposts.PostTitle AS posttitle, tblrecentposts.PostImage, tblcategory.CategoryName AS category, tblcategory.id AS cid, tblsubcategory.Subcategory AS subcategory, tblrecentposts.PostDetails AS postdetails, tblrecentposts.PostingDate AS postingdate, tblrecentposts.PostUrl AS url FROM tblrecentposts LEFT JOIN tblcategory ON tblcategory.id = tblrecentposts.CategoryId LEFT JOIN tblsubcategory ON tblsubcategory.SubCategoryId = tblrecentposts.SubCategoryId WHERE tblrecentposts.id = '$pid'");
            while ($row = mysqli_fetch_array($query)) {
            ?>
            <div class="overflow-hidden" style="height: 100%; max-height: 450px">
              <img class="card-img-top w-100 h-auto" src="admin/recentpostimages/<?php echo htmlentities($row['PostImage']); ?>" alt="<?php echo htmlentities($row['posttitle']); ?>">
