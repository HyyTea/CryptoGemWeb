...
{ 
    ...
    if (isset($_POST['update'])) {
        ...
        $uploadVideo = $_FILES['uploadvideo'];
        ...
        if (!empty($postVideo)) {
            ...
            if (strpos($postVideo, '<iframe') !== false) {
                ...
                if (isset($matches[1])) {
                    // Chuyển đổi URL iframe thành link YouTube
                    $postVideo = $matches[1]; // Sử dụng URL từ iframe
                    // Lấy ID video từ URL
                    if (preg_match('/youtube\.com\/embed\/([a-zA-Z0-9_-]+)/', $postVideo, $matches)) {
                        $videoId = $matches[1];
                        $postVideo = 'https://www.youtube.com/watch?v=' . $videoId; // Chuyển đổi thành link YouTube
                    }
                } else {
                ...
                // Không làm gì cả, giữ nguyên giá trị của $postVideo
            } else {
            ...
            if ($uploadVideo['error'] == UPLOAD_ERR_OK) {
                $uploadDir = 'featurepostvideo/'; // Thư mục lưu video
                $uploadFile = $uploadDir . basename($uploadVideo['name']);
                
                // Kiểm tra định dạng file
                $videoFileType = strtolower(pathinfo($uploadFile, PATHINFO_EXTENSION));
                $allowedTypes = array('mp4', 'avi', 'mov', 'wmv', 'flv');

                if (in_array($videoFileType, $allowedTypes)) {
                    // Di chuyển file đã upload đến thư mục đích
                    if (move_uploaded_file($uploadVideo['tmp_name'], $uploadFile)) {
                        $postVideo = $uploadVideo['name']; // Lưu tên file video vào biến
                    } else {
                        echo "<script>alert('Failed to upload video file.');</script>";
                        return;
                    }
                } else {
                    echo "<script>alert('Invalid video format. Please upload a valid video file.');</script>";
                    return;
                }
            }
