...
<!-- #region -->
    ...
    /* Adjust the spacing between image and text specifically for mobile */
    .col-md-2 .text-decoration-none div {
    ...
    <div class="row">
        ...
        <div class="mb-3">
            ...
            <div class="d-flex flex-wrap">
                ...
                // Các danh mục cần hiển thị
                $categories = ["Bitcoin", "Charts", "Ethereum", "Crypto Market", "Trading Terminology", "Trading News", "GameFi", "Analysis"];

                // Lấy danh sách category từ cơ sở dữ liệu
                $query = "SELECT id, CategoryName FROM tblcategory WHERE Is_Active = 1";
                $result = mysqli_query($con, $query);

                // Kiểm tra nếu có dữ liệu
                if (mysqli_num_rows($result) > 0) {
                    // Hiển thị các danh mục theo tên trong $categories
                    while ($row = mysqli_fetch_assoc($result)) {
                        $categoryId = $row['id'];
                        $categoryName = $row['CategoryName'];

                        // Kiểm tra nếu danh mục có trong danh sách $categories
                        if (in_array($categoryName, $categories)) {
                            echo '<div style="text-align: center;">';
                            echo '<a class="text-decoration-none" href="category.php?catid=' . htmlentities($categoryId) . '">';
                            echo '<p class="rounded-pill py-2 bg-secondary text-white me-2">' . htmlentities($categoryName) . '</p>'; // Hiển thị tên danh mục
                            echo '</a>';
                            echo '</div>';
                        }
                    }
            ...
            <div class="square p-3" style="border: 2px solid #ccc; margin: 10px; padding: 0; max-width: 100%; border-radius: 5%; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);">
                ...
                <div class="mb-2 d-flex justify-content-between hover-link">
                    <a target="_blank" href="https://www.facebook.com/bom.xuu">
                ...
                <div class="mb-2 d-flex justify-content-between hover-link">
                    <a target="_blank" href="https://x.com/hyyteahwang">
                        <div class="text-dark" style="display: flex; align-items: center;">
