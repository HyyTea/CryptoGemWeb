...
{ 
    ...
    if(isset($_POST['update'])) {
        ...
        $postVideo = $_POST['postvideo']; // Lấy URL video hoặc mã nhúng video từ form
    
        // Kiểm tra nếu người dùng nhập URL hoặc mã nhúng (iframe)
        if (!empty($postVideo) && (filter_var($postVideo, FILTER_VALIDATE_URL) || strpos($postVideo, '<iframe') !== false)) {
            ...
            if (filter_var($postVideo, FILTER_VALIDATE_URL)) {
                if (preg_match('/youtube\.com\/watch\?v=([a-zA-Z0-9_-]+)/', $postVideo, $matches)) {
                    $videoId = $matches[1];
                    // Thay thế VIDEO_ID bằng $videoId
        ...
        // Kiểm tra nếu người dùng upload tệp video
        elseif (isset($_FILES['uploadvideo']) && $_FILES['uploadvideo']['name'] != '') {
            $video_name = $_FILES['uploadvideo']['name'];
            $video_tmp = $_FILES['uploadvideo']['tmp_name'];
            $upload_dir = "featurepostvideo/";
            $video_path = $upload_dir . basename($video_name);
    
            // Kiểm tra và lưu file video vào thư mục
            if (move_uploaded_file($video_tmp, $video_path)) {
                $query = mysqli_query($con, "UPDATE tblfeaturevideo SET PostVideo='$video_name' WHERE id='$postid'");
                
                if($query) {
                    $msg = "Video file uploaded and post updated successfully";
                } else {
