...
<div class="py-5">
    ...
    <!-- Các video tiếp theo -->
    <div class="card featured-video" onclick="openLightbox()">
        <div class="row mt-5 justify-content-center">
            <?php foreach (array_slice($posts3, 1, 5) as $index => $post3) { ?>
                <div class="custom-col mb-4 d-flex justify-content-center" onclick="openLightbox('video-<?php echo $index; ?>')">
                    <div class="card">
                        <?php
    ...
    <!-- Modal for Lightbox -->
    <div id="videoLightbox" class="lightbox" onclick="closeLightbox()">
        <span class="close" onclick="closeLightbox()">&times;</span>
        <div class="lightbox-content">
            <iframe id="lightbox-video" class="small-video-frame" frameborder="0" allowfullscreen></iframe>
        </div>
    </div>
...
<style>
    ...
    .lightbox-content {
        position: relative;
        width: 80%;
        max-width: 800px; /* Chiều rộng tối đa của video */
    }

    .lightbox iframe {
        width: 100%;
        height: 450px; /* Hoặc chiều cao phù hợp khác */
    }
...
<script>
    ...
    // Lấy src của video được nhấp vào
    var videoSrc = document.getElementById(videoId).src; 
    document.getElementById('lightbox-video').src = videoSrc; // Sao chép src vào iframe trong modal
    document.getElementById('videoLightbox').style.display = "flex"; // Hiển thị modal với flex
}

function closeLightbox() {
    document.getElementById('videoLightbox').style.display = "none"; // Ẩn modal
    document.getElementById('lightbox-video').src = ""; // Dừng video khi đóng
}
