...
<div class="px-5 py-5">
    ...
    <div class="video-wrapper">
        <?php
        // Kiểm tra nếu PostVideo chứa đường link YouTube và chuyển đổi sang mã nhúng
        if (strpos($featuredVideo['PostVideo'], 'youtube.com/watch?v=') !== false) {
            $videoEmbedUrl = str_replace('watch?v=', 'embed/', $featuredVideo['PostVideo']);
        ...
        <?php foreach (array_slice($posts3, 1, 5) as $index => $post3) { ?>
            <div class="custom-col mb-4 d-flex" onclick="playVideo('video-<?php echo $index; ?>')">
                <div class="card">
                    ...
                    <?php
                    // Chuyển đổi đường link YouTube sang mã nhúng
                    if (strpos($post3['PostVideo'], 'youtube.com/watch?v=') !== false) {
                        $videoEmbedUrl = str_replace('watch?v=', 'embed/', $post3['PostVideo']);
