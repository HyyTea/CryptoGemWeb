        ...
        <div class="col-md-9">
            ...
            <div class="row">
                ...
                foreach ($posts as $index => $post) {
                    ...
                    echo '<p>' . htmlentities(substr($post['postdetails2'], 0, 100)) . '...</p>'; // Cắt ngắn nội dung
...
<div class="px-5 py-5">
    ...
    <div class="row" style="gap: 20px;">
        ...
        foreach (array_slice($posts2, 0, 5) as $index => $post2) {
            ...
            echo '<p style="font-size: 14px;">' . htmlentities(substr($post2['postdetails4'], 0, 100)) . '...</p>'; // Rút gọn nội dung
...
<div class="px-5 py-5">
    ...
    <?php
    // Kiểm tra nếu dữ liệu trong $featuredVideo['PostVideo'] chứa mã nhúng (iframe) hay URL
    if (strpos($featuredVideo['PostVideo'], '<iframe') !== false) {
        // Xuất mã nhúng trực tiếp nếu là iframe
        echo $featuredVideo['PostVideo'];
    } elseif (strpos($featuredVideo['PostVideo'], 'youtube.com') !== false || strpos($featuredVideo['PostVideo'], 'vimeo.com') !== false) {
        // Xử lý URL video nếu là YouTube hoặc Vimeo
        ...
        echo '<iframe class="featured-video-iframe" src="' . htmlentities($videoEmbedUrl) . '" frameborder="0" allowfullscreen></iframe>';
    } else {
        // Nếu là file tải lên
        echo '<video class="featured-video-frame" preload="none" controls>
                <source src="featurepostvideo/' . htmlentities($featuredVideo['PostVideo']) . '" type="video/mp4">
              </video>';
    }
    ?>
</div>
            ...
            <div class="custom-col mb-4 d-flex" onclick="playVideo('video-<?php echo $index; ?>')">
                <div class="card">
                    ...
                    if (strpos($post3['PostVideo'], 'youtube.com') !== false || strpos($post3['PostVideo'], 'vimeo.com') !== false) {
                        ...
                        echo '<iframe id="video-' . $index . '" class="small-video-frame" src="' . htmlentities($videoEmbedUrl) . '" frameborder="0" allowfullscreen></iframe>';
                    } else {
